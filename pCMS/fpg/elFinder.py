...
class connector():
    ...
    def __init__(self, opts):
        ...
        self._options['root'] = self._options['root'].rstrip('/')
    ...
    def __rename(self):
        ...
        if 'name' in self._request and 'current' in self._request and 'target' in self._request:
            ...
            newName = os.path.join(smart_unicode(curDir), name)
    ...
    def __mkdir(self):
        ...
        if 'name' in self._request and 'current' in self._request:
            ...
            newDir = os.path.join(smart_unicode(path), name)
    ...
    def __mkfile(self):
        ...
        if 'name' in self._request and 'current' in self._request:
            ...
            current = self._request['current']
            curDir = self.__findDir(current, None)
            newFile = os.path.join(smart_unicode(curDir), name)
        ...
        if not curDir or not name:
            ...
            try:
                ...
                self.__content(smart_unicode(curDir), False)
            except:
            ...
            for upFile in upFiles:
                if upFile.name:
                    ...
                    if not self.__checkName(name):
                        ...
                        name = os.path.join(smart_unicode(curDir), name)
                        try:
    ...
    def __resize(self):
        ...
        try:
            ...
            self.__debug('resizeFailed_', str(e))
    ...
    def __thumbnails(self):
        ...
        if self.__canCreateTmb():
            ...
            i = 0
            for f in os.listdir(smart_unicode(curDir)):
    ...
    def __tree(self, path):
        ...
        if self.__isAllowed(path, 'read'):
            for d in sorted(os.listdir(smart_unicode(path))):
    ...
    def __remove(self, target):
        ...
        if not os.path.isdir(target):
            ...
            for i in os.listdir(smart_unicode(target)):
    ...
    def __copy(self, src, dst):
        ...
        if not os.path.isdir(src):
            ...
            for i in os.listdir(smart_unicode(src)):
    ...
    def __findDir(self, fhash, path):
        ...
        for d in os.listdir(smart_unicode(path)):
            pd = os.path.join(path, d)
    ...
    def __find(self, fhash, parent):
        ...
        if os.path.isdir(parent):
            for i in os.listdir(smart_unicode(parent)):
        ...
        if 'current' in self._request and 'target' in self._request and 'content' in self._request:
            ...
            if curFile and curDir:
                if self.__isAllowed(curFile, 'write'):
                    try:
                        f = open(curFile, 'w+')
                        f.write(self._request['content'].encode('utf-8'))
                        f.close()
    ...
    def __hash(self, path):
        """Hash of the path"""
        m = hashlib.md5()
        m.update(u"%r"%smart_unicode(path))
